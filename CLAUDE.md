...
- **学校全称**：东莞中学；英文名只能写 **Dongguan High School**。
- **命名红线**：不要写成“东华中学”、`Donghua High School`、`GZ`、`gz` 或任何会让人误解为其他学校/城市的简称。“莞中”可以作为中文语境下的非正式简称，但正式描述、站点元信息、logo、README、部署说明和对外传播文案必须使用“东莞中学 / Dongguan High School”。
- **对标**：上交生存手册（结构化方法论）+ 清华/上交飞跃手册（按主题组织）
- **受众**：莞中在校生 + 莞中校友 + 莞中教师 （不包含莞中家长）
- **品牌关系**：与"莞中人指北"（非公开共享文档）是兄弟产品。指北放群内敏感内容，飞跃手册放公网结构化内容
...
飞跃手册不是公众号文章合集，也不是把非公开原文搬到公网。它应该是一个长期内容母库：主站负责保存结构化、可检索、可维护的正文；非公开材料池继续承载群内才适合看的真实材料。

| 渠道 | 角色 | 可以放什么 | 不应该放什么 |
|---|---|---|---|
| 非公开材料池 | 原始材料池 / 非公开讨论场 | 群内经验、未脱敏反馈、征集草稿、敏感但有价值的提醒 | 直接面向公网的定稿 |
...
- 敏感内容、未授权材料、只适合非公开讨论的内容，不因为公众号需要而进入公网。
...
- **不会 Git 或只愿意随手写的人**：先通过问卷、微信群收集成原始材料，由维护者判断它属于案例、观点、主干页素材，还是只适合留在指北。
...
1. 收集：通过 Git PR、问卷、微信群、私聊等方式收集原始材料。
2. 分流：维护者判断材料进入 `cases/entries/`、局部 `perspectives/`、B 类主干页，或保留在非公开材料池。
...
| 层 | 第一版选择 | 原因 | 暂不选择 |
|---|---|---|---|
| 内容格式 | Markdown + YAML frontmatter + LaTeX 数学公式 | 容易 PR、容易迁移、适合长期版本管理；公式用 MathJax 渲染 | 数据库 CMS、在线文档直接做公网正本；整篇 `.tex` 作为内容源 |
| 静态站 | MkDocs Material | 中文搜索、导航、移动端、部署成熟，适合知识库 | GitBook 作为主站、从零写前端 |
| 仓库正本 | Git 仓库 | 保留历史、方便 review、方便自动构建 | 群文件或在线文档作为正本 |
| 投稿入口 | Git PR + 问卷/微信收稿 | 同时照顾会 Git 和不会 Git 的校友 | 只允许 PR |
...
| 私密内容 | 留在非公开材料池 | 真正区分公开内容和群内内容 | 给静态站做伪登录 |
